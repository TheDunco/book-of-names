...
export const backpopulateAfterChangeHookFactory = ({
    ...
    const hook: FieldHook = async (args) => {
        ...
        if (operation === 'create' || operation === 'update') {
            ...
            const removedTargetIds = previousValue ? [...previousValue].filter((x) => !value.includes(x)) : [];
            const addedTargetIds = value.filter((x: any) => !(previousValue ?? []).includes(x));
            ...
            for (const documentToRemoveBackPop of documentsToRemoveBackPop) {
                ...
                const prevReferencedIds = documentToRemoveBackPop[backpopulatedField['name']].map(
                    (doc: { id: any }) => doc.id
                );

                const updatedReferenceIds = prevReferencedIds.filter((doc: { id: any }) => {
            ...
            for (const documentToAddBackPop of documentsToAddBackPop) {
                const prevReferencedIds = documentToAddBackPop[backpopulatedField['name']].map(
                    (doc: { id: any }) => doc.id
                );
                const updatedReferenceIds = Array.from(new Set([...prevReferencedIds, originalDoc.id]));
                await payload.update({
                    collection: targetCollection.slug,
